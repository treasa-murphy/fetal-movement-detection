...
def label_samples(positive_samples, negative_samples):
    """labels samples as positive (1) or negative (0) and combines them."""
...
def extract_features(window_data, channels=('p1')):
    """extracts piezo features (p1, p4 or both) from a single window."""
    # drop unnecessary columns and extract signal data
    features = window_data.reset_index().drop(
        columns=['s3key', 'study_id', 'sample_num', 'measurement_index', ('button', 'button')]
    )
    extracted = []

    if 'p1' in channels:
        extracted.append(features[('piezos', 'p1')].values.reshape(-1, 1))
    if 'p4' in channels:
        extracted.append(features[('piezos', 'p4')].values.reshape(-1, 1))

    if extracted:
        return np.hstack(extracted)
    else:
        raise ValueError("no valid channels specified. use 'p1', 'p4', or both.")
